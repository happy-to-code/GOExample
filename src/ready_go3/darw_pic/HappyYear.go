...
var (
	...
	hwFontTtf *truetype.Font // 字体
)

func main() {
	// 根据路径打开模板文件
	templateFile, err := os.Open("C:\\mydata\\project\\golang_test\\GOExample\\src\\ready_go3\\darw_pic\\file\\template_2ys.png")
	if err != nil {
		panic(err)
	}
	...
	// 华文行楷
	hwFontTtf, err = loadFont("C:\\mydata\\project\\golang_test\\GOExample\\src\\ready_go3\\darw_pic\\file\\STXINGKA.TTF")
	if err != nil {
		log.Panicln(err.Error())
		return
	}
	...
	// ====================向模板中粘贴图片 begin========================
	// 		1、根据地址获取图片内容
	imageData, err := getDataByUrl("C:\\mydata\\project\\golang_test\\GOExample\\src\\ready_go3\\darw_pic\\file\\log1.png")
	if err != nil {
	...
	draw.Draw(newTemplateImage,
		transparentImg.Bounds().Add(image.Pt(84, 400)),
		transparentImg,
...
func writeWord2Pic(newTemplateImage *image.RGBA) {
	...
	content.SetFontSize(20)    // 设置字体大小
	content.SetFont(hwFontTtf) // 设置字体样式，就是我们上面加载的字体

	// 	正式写入文字
	// 参数1：要写入的文字
	// 参数2：文字坐标
	content.SetSrc(image.NewUniform(color.RGBA{R: 206, G: 162, B: 115, A: 255}))
	content.DrawString("春日笑颜陈新岁，", freetype.Pt(490, 500))
	content.DrawString("研心福满家团圆。", freetype.Pt(490, 500+27+27))
	content.DrawString("君风祺福长久在，", freetype.Pt(490, 500+27+27+27+27))
	content.DrawString("笑语祝愿幸福缠。", freetype.Pt(490, 500+27+27+27+27+27+27))
	...
	//
	//content.SetFont(hwFontTtf) // 设置字体样式
	//// 设置字体大小
...
func saveFile(pic *image.RGBA) {
	dstFile, err := os.Create("C:\\mydata\\project\\golang_test\\GOExample\\src\\ready_go3\\darw_pic\\dst\\dst.png")
	if err != nil {
		fmt.Println(err)
	}
	defer dstFile.Close()
