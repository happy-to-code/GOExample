...
var (
	...
	database = pflag.StringP("database", "d", "gormtest", "Database name to use")
...
func main() {
	...
	if err := db.Create(&Product{Code: "D55", Price: 1500}).Error; err != nil {
	...
	if err := db.Where("code = ?", "D41").Delete(&Product{}).Error; err != nil {
		log.Fatalf("Delete product error: %v", err)
	}
	PrintProducts(db)
}

// PrintProducts List products
