...
func main() {
	...
	for i := 0; i < 15; i++ {
		...
		api = api + "?bac045=&aae135=" + param6.Paperid + "&baz805&aac001=&date=20210818&time=142623&cpage=0&rows=120"
		ylbx := httpDo(httpClient, "POST", api, contentType, "")
		fmt.Println("养老保险：", ylbx)

		// 养老保险
		// res := new(PensionRes)
		// 1.json.Unmarshal([]byte(ylbx), res)
		// if res.Code == 200 {
		// 	pensionRes := handPensionRes(res.Data)
		// 	bytes, _ := 1.json.Marshal(pensionRes)
